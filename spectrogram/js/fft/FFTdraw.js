let viridis = [[0.267004, 0.004874, 0.329415],
...
class _fftSpectrogramDisplay {
  ...
  getColor(d) {
    ...
    // return (`rgb(${Math.min(d**3, 10) + d/2}, ${d*2 - 255}, ${d*4 - 255*2})`);
    // return (`rgb(${Math.min(Math.max(d*5 - 300, 0), 155) + Math.max(Math.min(d-180, 70), 0) + Math.min(d**2, 30)}, ${d + Math.min(d**2, 30)}, ${Math.min(d**3, 80) + d*0.82 + Math.min(d**2, 30)})`);
  }
  setPause(bool) {
    this.paused = bool;
  }
